"""Nine-method farm comparison; remove only three update-control rows and their space."""
...
ROOT=HERE.parents[1]
DATA=ROOT/'results/current_figure_data/figure09'
OUT=ROOT/'outputs/current/figure09'
OUT.mkdir(parents=True, exist_ok=True)
FIG=OUT
OLD=DATA/'reference_data'
SOURCE=DATA/'farm_means.csv'
data=pd.read_csv(SOURCE)
primary=['CLARA_online','TSC','LinUCB','CART','EEE','EnbPI-RH','ACI','Static','AgACI']
controls=[]
methods=primary+controls
names={'CLARA_online':'CLARA'}
pal=json.loads((DATA/'method_palette.json').read_text())
colors={m:pal.get(names.get(m,m),'#A8B9BD') for m in methods}
colors.update(CLARA_online=pal['CLARA'])
from _fonts import preferred_font
preferred_font()
mpl.rcParams.update({
    'font.family':'Times New Roman','font.size':8.5,'axes.labelsize':8.5,'axes.titlesize':9,
...
WIDTH_MM,HEIGHT_MM=183,143
fig=plt.figure(figsize=(7.20472440945,5.62992125984))
...
for k,m in enumerate(methods):
    ...
    for farm,mark,dy in [('Farm A','o',-.12),('Farm B','^',.12)]:
...
for i,m in enumerate(primary):
    mlabel.text(.5,i,names.get(m,m),ha='center',va='center',fontsize=8.5,
                fontweight='bold' if i==0 else 'normal')
title(XLEFT,72.7,'c','Cost across prices')
title(XRIGHT,72.7,'d','Undercoverage across prices')
for x,im,label in [(XLEFT,ims[0],'ERRF above the price-specific minimum (%)'),(XRIGHT,ims[1],'TUWR (%)')]:
    cb=fig.colorbar(im,cax=axes_mm(x,129.5,COLWIDTH,1.6),orientation='horizontal')
...
old_summary=pd.read_csv(OLD/'Figure8_summary_data.csv').set_index('method').loc[methods].reset_index()
pd.testing.assert_frame_equal(src,old_summary,check_names=False,rtol=1e-13,atol=1e-13)
...
fig.savefig(FIG/'Figure9_farm_comparison.pdf')
fig.savefig(FIG/'Figure9_farm_comparison.svg')
fig.savefig(FIG/'Figure9_farm_comparison.png',dpi=600)
fig.savefig(FIG/'Figure9_farm_comparison.tiff',dpi=600,pil_kwargs={'compression':'tiff_lzw'})
fig.savefig(FIG/'Figure9_preview.png',dpi=300)
(OUT/'figure_qa.json').write_text(json.dumps({
    'status':'PASS','source_sha256':hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
    'retained_data_identical_to_approved_figure':True,'primary_methods':9,'controls':0,'prices':5,'farms':2,
    'width_mm':WIDTH_MM,'height_mm':HEIGHT_MM,'previous_height_mm':160,
    'height_reduction_pct':100*(160-HEIGHT_MM)/160,'row_outlines':0,
...
print('Figure 9 exported; nine-method data unchanged; three controls removed; column alignment verified.',flush=True)
