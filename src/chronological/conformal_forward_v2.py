# Prospective rerun: preserve the main calibrator ten-decimal alpha convention.
...
def _linear_quantile(values: np.ndarray, start: int, end: int, quantile: float) -> float:
    ...
    if fraction >= 0.5:
        return float(ordered[upper_index] - (1.0 - fraction) * (ordered[upper_index] - ordered[lower_index]))
...
def _evaluate_grid(
    ...
    for task_index in range(configuration_count * coverage_count):
        ...
        alpha = np.round(1.0 - coverage, 10)
        ...
        for event_index in range(event_count):
            ...
            if family == 0:
                ...
                margin = max(
                    0.0,
                    _linear_quantile(history_values, history_start, history_end, 1.0 - alpha),
                )
...
def evaluate_conformal_grid_compact(
    *,
    ...
    static_margins = np.asarray(
        [
            max(0.0, float(np.quantile(calibration_scores[index], 1.0 - round(1.0 - float(coverage), 10), method="linear")))
